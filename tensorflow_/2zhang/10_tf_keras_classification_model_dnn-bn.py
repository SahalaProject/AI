...
"""
深度神经网络 中 添加 批归一化（BatchNormalization）
加入批归一化后
前期曲线有变化，训练有效果
...
for _ in range(20): # 通过for循环添加20层 神经网络
    model.add(keras.layers.Dense(100, activation='relu')) # 定义每一层大小100
    model.add(keras.layers.BatchNormalization())  # 实现批归一化 （keras中将批归一化作为单独的层次）， 几乎函数在批归一化前实现
    """
    一种 将激活函数 activation 放入批归一化 之后的实现， 两种实现都可以， 缓解梯度下降
    model.add(keras.layers.Dense(100))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.Activation('relu))   # 激活函数activation在 keras中 也被放入一个层次
    """
...
history = model.fit(x_train_scaled, y_train, epochs=50, validation_data=(x_valid_scaled, y_valid), callbacks=callbacks) # epochs遍历数据集的次数, 每隔一段时间将会对验证集做验证

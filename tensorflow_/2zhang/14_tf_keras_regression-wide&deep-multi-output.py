...
"""
 wide & deep模型 的 多输出   ,将hidden2经过全连接层输出 model 、evaluate 和 fit 需要两个输出
...
output2 = keras.layers.Dense(1)(hidden2) # 将hidden2经过全连接层输出 model 、evaluate 和 fit 需要两个输出

model = keras.models.Model(inputs=[input_wide, input_deep], outputs=[output, output2]) # 两个输出
# fit前需要拆分模型
...
# 训练, 多输入x替换 ， 多输出 两个输出 y_train 改 [y_train, y_train], y_valid 改 [y_valid, y_valid]
...
polt_learning_curves(history) # 打印值训练值变化图， 多输出 有6条曲线线

# 测试模型
test_result = model.evaluate([x_test_scaled_wide, x_test_scaled_deep], [y_test, y_test]) # 多输出 两个输出 y_test 改 [y_test, y_test]
print(test_result)
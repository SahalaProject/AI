...
#############################wide&deep###########################################
# 搭建模型
# 因为wide & deep模型不是层级模型，而是有两部分组成，所以不能用 Sequential方式，需使用函数式API （读 c kong sha）
input = keras.layers.Input(shape=x_train.shape[1:]) # 输入层
# 指定层次keras.layers.Dense(units=30, activation='relu')相当于函数的层次将(input)输入，得到输出（这就是函数式API特性）
hidden1 = keras.layers.Dense(units=30, activation='relu')(input)  # 用两层模型实现deep model,
hidden2 = keras.layers.Dense(units=30, activation='relu')(hidden1) # 这种形式类似 复合函数：f(x) = h(g(x))

concat = keras.layers.concatenate([input, hidden2]) # 拼接 wide 输入 和 deep 输出
output = keras.layers.Dense(1)(concat)

# 通过model 将模型固化下来
model = keras.models.Model(inputs = [input], outputs = [output])

# 如下网络结构： input_1  输入给 dense， dense和input_1输入给dense_1， 以此类推
# Model: "functional_1"

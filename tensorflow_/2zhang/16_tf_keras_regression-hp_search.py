...
"""
手动实现 超参数搜索  (不推荐，非并行、多个搜索时for层数增加等，请看17)
...
# w = W + grad * learning_rate # 求导
learning_rate =  [1e-4, 3e-4,1e-3, 3e-3, 1e-2, 3e-2] # 定义learning_rate集合
# 保留所有模型的historys
historys = []
for lr in learning_rate:
    ...
    # 要使用learning_rate， 需要定义我们自己的optimizer
    optimizer = keras.optimizers.SGD(lr)

    # print(model.summary())
    # 编译模型  计算目标函数
    # model.compile(loss='mean_squared_error', optimizer=keras.optimizers.SGD(0.001)) # 编译model
    model.compile(loss='mean_squared_error', optimizer=optimizer) # 使用我们自己的optimizer
    # 会调
...
###################################################################################################
    ...
    # 打印保存的所有historys
    for lr, history_ in zip(learning_rate, historys): # 将learning_rate, historys组合起来
        print('Learning_rate: ', lr) # 打印出lr， 对应下面的曲线图
        polt_learning_curves(history_) # 打印值训练值变化图
